...
"""Synchronous vLLM rollout for the FSDP training path."""
...
def _pre_process_inputs(pad_token_id, prompt_token_ids: torch.Tensor) -> List[int]:
    # Remove left padding before sending prompt token ids to vLLM.
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if self.sampling_params.n > 1 and do_sample:
                ...
                if "trace_id" in non_tensor_batch.keys():
                    non_tensor_batch["trace_id"] = _repeat_interleave(non_tensor_batch["trace_id"], self.sampling_params.n)
                if "extra_info" in non_tensor_batch.keys():
                    non_tensor_batch["extra_info"] = _repeat_interleave(non_tensor_batch["extra_info"], self.sampling_params.n)

...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name != "vllm":
            raise NotImplementedError("UMEM open-source training supports vLLM rollout only.")
        if self.config.rollout.mode != "sync":
            raise NotImplementedError("UMEM open-source training supports sync vLLM rollout only.")
        ...
        rollout = vLLMRollout(model_path=local_path, config=self.config.rollout, tokenizer=self.tokenizer, model_hf_config=self.actor_model_config, device_mesh=rollout_device_mesh, trust_remote_code=trust_remote_code, **lora_kwargs)
...
class RetrieverWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_model(self):
        from verl.workers.retriever.dp_retriever import DataParallelRetriever
        self.retriever = DataParallelRetriever(self.config)

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def retrieval(self, data: DataProto):
        return self.retriever.compute_retrieval(data)

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def encode(self, data: DataProto):
        return self.retriever.compute_encode(data)

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def search(self, data: DataProto):

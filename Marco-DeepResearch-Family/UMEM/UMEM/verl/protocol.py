...
def pad_dataproto_to_divisor(data: "DataProto", size_divisor: int):
    """Pad a DataProto by repeating leading samples until the size is divisible by size_divisor."""
    ...
    while remaining_pad > 0:
        ...
        part = data[:take_size]
        padding_protos.append(part)
        remaining_pad -= take_size

    padding_data = DataProto.concat(padding_protos)

    # =================================================================

    # =================================================================

    if hasattr(padding_data, "batch") and padding_data.batch is not None:

        if "response_mask" in padding_data.batch:
            padding_data.batch["response_mask"] = torch.zeros_like(padding_data.batch["response_mask"])

        # if "attention_mask" in padding_data.batch:
        #     padding_data.batch["attention_mask"] = torch.zeros_like(padding_data.batch["attention_mask"])

        if "loss_mask" in padding_data.batch:
            padding_data.batch["loss_mask"] = torch.zeros_like(padding_data.batch["loss_mask"])

        if "advantages" in padding_data.batch:
            padding_data.batch["advantages"] = torch.zeros_like(padding_data.batch["advantages"])

    data_padded = DataProto.concat([data, padding_data])

    return data_padded, pad_size

def _pad_dataproto_to_divisor(data: "DataProto", size_divisor: int):
    ...
    def pop(self, batch_keys=None, non_tensor_batch_keys=None, meta_info_keys=None) -> "DataProto":
        ...
        for key in non_tensor_batch_keys:
            if key not in self.non_tensor_batch.keys():
                raise AssertionError(f"non_tensor_batch key not found: {key}")

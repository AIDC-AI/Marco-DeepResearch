...
class BaseCheckpointManager:
    ...
    def remove_previous_save_local_path(self, path):
        ...
        for p in path:
            ...
            logger.info("Removing previous checkpoint path: %s", abs_path)
...
def find_latest_ckpt_path(path, directory_format="global_step_{}"):
    ...
    if not os.path.exists(tracker_file):
        logger.info("Checkpoint tracker file does not exist: %s", tracker_file)
        return None
    ...
    if not os.path.exists(ckpt_path):
        logger.info("Checkpoint does not exist: %s", ckpt_path)
        return None

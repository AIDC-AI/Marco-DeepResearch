...
class Profiler:
    ...
    def __init__(self, config):
        ...
        if config.use_profile and self.rank in self.config.profile_ranks:
            logger.info("Profiler init for rank %s", self.rank)
    ...
    def _validate(self):
        if self.config.use_profile:
            if self.config.profile_ranks is None:
                logger.warning("Profile ranks is not set, defaulting to rank 0")
    ...
    def start(self):
        if self.check():
            logger.info("Profiler started for rank %s", self.rank)
            self.prof.start()
    ...
    def stop(self):
        if self.check():
            logger.info("Profiler stopped for rank %s", self.rank)
            self.prof.stop()
    ...
    def save(self):
        if self.prof is not None and not self.saved:
            ...
            logger.info("Saving profiler trace to %s", self.config.save_path + save_file_name)
    ...
    def stop_trace(self):
        if self.check():
            logger.info("Profiler trace stopped for rank %s", self.rank)
            self.skip_prof = True
